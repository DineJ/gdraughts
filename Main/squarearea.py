...
class SquareArea(Gtk.DrawingArea): 
	def __init__ (self, name=None, color=1.0, square=20, square_type=0, pawn_color=0.0):
	...
	def do_release_mouse(self, widget, event): #
		print ("Mouse clicked... at %d, %d (%s)" % (event.x, event.y, self.name))
	...
	def draw_pawn(self, cr, width, height): 
		...
		cr.set_source_rgb(self.pawn_color, self.pawn_color, self.pawn_color)
		cr.fill()
		cr.arc(0, 0, radius, 0.0, 2.0 * math.pi)
	...
	def draw_queen(self, cr, width, height): 
		...
		cr.fill()
		cr.arc(0, 0, radius, 0.0, 2.0 * math.pi)
		cr.set_source_rgb(1 - self.pawn_color, 1 - self.pawn_color, 1 - self.pawn_color)
		cr.set_line_width(6)
		cr.stroke()
